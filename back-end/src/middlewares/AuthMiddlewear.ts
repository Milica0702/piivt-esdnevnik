...
export default class AuthMiddleware {
    public static getVerifier(... allowedRoles: ("student"|"professor")[]): (req: Request, res: Response, next: NextFunction) => void {
        return (req: Request, res: Response, next: NextFunction) => {
            this.verifyAuthToken(req, res, next, allowedRoles);
        }
    }
    private static verifyAuthToken(req: Request, res: Response, next: NextFunction, allowedRoles: ("student"|"professor")[]) {
    ...
    public static validateTokenAs(tokenString: string, role: "student" | "professor", type: "auth" | "refresh"): ITokenData {
        if (tokenString === "") {
            throw {
                status: 400,
